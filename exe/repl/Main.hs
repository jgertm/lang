...
handle (Expression e) = do
  ...
  putStrLn $ unlines $ catMaybes $ case interpretWith definitions ast of
    ...
    Right exp ->
      ...
      in  [ Just (show exp)
          , if showType flags then Just typ else Nothing
          , if showConstraints flags
            then Just (unlines $ map show constraints)
            else Nothing
          , if showAST flags then Just $ show $ stripContext ast else Nothing
          ]
handle (Definition d) = case d of
